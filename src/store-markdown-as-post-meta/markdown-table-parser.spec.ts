import { parseParagraph, parseDelimiters, parseTableRow } from './markdown-table-parser';

describe.only('parseParagraph', () => {
    it('should parse a paragraph with only a table', () => {
        const generator = parseParagraph(`| Header 1 | Header 2 |
| -------- | -------- |
| Row 1    | Row 2    |
`);
    ...
    it('should parse a paragraph with trailing text and a table', () => {
        const generator = parseParagraph(`The open source publishing platform of choice for millions of 
...
`);
        expect(Array.from(generator)).toEqual([
            {
                type: "text",
                text: "The open source publishing platform of choice for millions of websites worldwide—from creators and small businesses to enterprises.",
            },
            {
                type: 'table',
                headers: ['Header 1', 'Header 2'],
...
describe('parseTableRow', () => {
    ...
    it('should parse a row with textual contents', () => {
        const parsed = parseTableRow(`| this is left column | and right column |`, 0);
        expect(parsed?.cells).toEqual([
            'this is left column',
            'and right column',
        ]);
